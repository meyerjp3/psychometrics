...
public class CronbachAlpha implements ScoreReliability, Comparable<CronbachAlpha>{
	...
	public String print(boolean unbiased){
		...
		f.format("%19s", "Cronbach's Alpha = "); f.format(f2,this.value(unbiased));
		return f.toString();
	}
    ...
    public String ifDeletedToString(ArrayList<VariableInfo> var){
        ...
        f.format("%-56s", " Cronbach's Alpha (SEM in Parentheses) if Item Deleted"); f.format("%n");
	...
	public double value(boolean unbiased){
		...
		cronbachAlpha = (k/(k-1.0))*((observedScoreVariance-componentVariance)/observedScoreVariance);
		return cronbachAlpha;
	}
	
	public int compareTo(CronbachAlpha that){
		if(this.cronbachAlpha>that.cronbachAlpha) return 1;
		if(this.cronbachAlpha<that.cronbachAlpha) return -1;
		return 0;
	}
	...
	public boolean equals(Object obj){
		...
		Double v = new Double(cronbachAlpha);
		return ((Double)obj)==v;
